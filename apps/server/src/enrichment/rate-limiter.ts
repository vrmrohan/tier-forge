...
// process agrees on "now". Returns how long the caller must wait.
...
export function createRedisRateLimiter(
  ...
  return {
    async acquire(signal) {
      ...
      let waitMs: number;
      try {
        waitMs = Number(await redis.eval(RESERVE_SCRIPT, 1, key, interval));
      } catch (error) {
...
export function createInMemoryRateLimiter(
  ...
  return {
    async acquire(signal) {
      const reservation = reserveSlot(now(), nextFreeMs, interval);
      nextFreeMs = reservation.nextFreeMs;
      await sleep(reservation.waitMs, signal);
    },
    async pause(ms) {
